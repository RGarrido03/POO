...
public class POO2223 {
	...
	private static void alinea2(PrintStream out) {
		...
		*/
		...
		try (Scanner input = new Scanner(new File("resources/bookings.txt"))) {
			input.useDelimiter("[\\n\\r;]+");
			while (input.hasNext()) {
				// Discard line if starts with #. Otherwise, use it for client nif.
				...
				int clientNif = Integer.parseInt(first);
				...
				UnitType unitType = UnitType.valueOf(input.next());
				int[] dimensions = {
						Integer.parseInt(input.next("x")),
						Integer.parseInt(input.next("x")),
						Integer.parseInt(input.next())
				};

				storageService.rentStorageUnit(
						storageService.getClient(clientNif),
						storageService.findAvailableUnits(unitType, startDate, duration, dimensions).get(0),
						startDate,
						duration
				);
			}
		...
		// Não executei a ver se funciona.
